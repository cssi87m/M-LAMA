...
def load_checkpoint_with_compatibility(model: torch.nn.Module, checkpoint_path: str, device: str = "cpu",
...
def main():
    ...
    model = ESLGradingModelByCandidatesWithAudio(
        ...
        audio_encoder_id=config.model.audio_encoder_id,
        audio_encoder_type=config.model.audio_encoder_type,  # NEW
        audio_encoder_frozen=config.model.audio_encoder_frozen,  # NEW
        pooling_dropout=config.model.pooling_dropout,
        regression_dropout=config.model.regression_dropout,
        avg_last_k=config.model.avg_last_k,
        d_fuse=config.model.d_fuse,
        adapter_bottleneck_dim=config.model.adapter_bottleneck_dim,
        adapter_num_heads=config.model.adapter_num_heads,
        adapter_dropout=config.model.adapter_dropout,
        num_parts=config.model.num_parts,
        hierarchical_audio_pooling=config.model.hierarchical_audio_pooling,
        use_enhanced_hierarchical=config.model.use_enhanced_hierarchical,  # NEW
        use_gated_fusion=config.model.use_gated_fusion,  # PHASE 2
        num_self_attn_layers=config.model.num_self_attn_layers,  # PHASE 2
        use_question_encoder=config.model.use_question_encoder,  # STEP 3
    )
    ...
    ckpt = None
    if resume_ckpt:
        # NEW: Use enhanced checkpoint loading with encoder change detection
        ...
        if ckpt_path.exists():
            ...
            start_epoch = 0  # Reset for fine-tuning
