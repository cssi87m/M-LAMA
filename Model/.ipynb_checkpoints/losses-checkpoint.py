...
def compute_combined_loss(outputs, true_scores, weights, config):
    """
    Compute combined loss: Focal + Ranking + Distribution-Aware

    Args:
        outputs: Dict with 'logits' [B, 21] and 'expected_score' [B]
        true_scores: [B] - true scores
        weights: [B] - class weights
        config: TrainingConfig with loss parameters
