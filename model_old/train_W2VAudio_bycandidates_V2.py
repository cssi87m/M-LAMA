...
class ESLDatasetByCandidatesWithAudio(Dataset):
    def __init__(self, dataframe, criteria='grammar', audio_processor=None, remove_low_content=True, num_chunks=10, chunk_length_sec=30):
        """
        Enhanced ESL Dataset that supports both text and audio.

        Args:
            dataframe: DataFrame with columns 'text', scoring columns, 'question_type', 'absolute_paths'
            criteria: Scoring criterion to use ('grammar', 'vocabulary', 'fluency', etc.)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for batch in pbar:
                ...
                pbar.set_postfix({
                    'loss': f'{loss.item():.4f}',
                    'kl': f'{weighted_kl_loss.item():.4f}',
                    'mse': f'{weighted_mse.item():.4f}'
                })
                ...
                if global_step % 10 == 0:
                    wandb.log({
                        ...
                        'train/step_kl_loss': weighted_kl_loss.item(),
                        'train/step_mse_loss': weighted_mse.item(),
            ...
            wandb.log({
                ...
                'train/epoch_mse_loss': avg_mse_loss,
                'val/weighted_mse': val_w_loss,
                'val/average_mse': val_avg_loss,
                'val/best_weighted_mse': best_val_loss
            }, step=global_step)

            if val_w_loss < best_val_loss:
                best_val_loss = val_w_loss
                best_state_dict = {k: v.detach().cpu() for k, v in self.model.state_dict().items()}
                print(f"New best model! Validation MSE: {best_val_loss:.4f}")
                # Log best model update
                wandb.log({'val/new_best': 1}, step=global_step)
            elif val_w_loss > best_val_loss * 1.1:
                self.model.load_state_dict(best_state_dict)
                print("Current model is too bad; reloading best validation model.")
                wandb.log({'val/model_reverted': 1}, step=global_step)
...
class ESLTrainerByCandidatesWithAudio(ESLTrainerByCandidates):
    def __init__(self,
                 ...
                 model,
                 tokenizer,
                 criteria='grammar',  # NEW: scoring criterion
        ...
        self.criteria = criteria  # Store criterion
        self.audio_processor = audio_processor
    ...
    def _prepare_data(self):
        """Override data preparation to include audio and criterion"""
        train_df = pd.read_csv(self.train_path)
        val_df = pd.read_csv(self.val_path)
        test_df = pd.read_csv(self.test_path)
        ...
        sampling_alpha = 0.2  # Reduced since data is manually balanced
        train_dataset = ESLDatasetByCandidatesWithAudio(train_df, self.criteria, self.audio_processor)
        train_sampler = InverseScoreSampler(train_dataset, alpha=sampling_alpha)
        self.train_loader = DataLoader(
            train_dataset,
            batch_size=self.batch_size,
        ...
        self.val_loader = DataLoader(
            ESLDatasetByCandidatesWithAudio(val_df, self.criteria, self.audio_processor),
            batch_size=self.batch_size,
            collate_fn=collate_fn,
            num_workers=16,
            prefetch_factor=2,
        ...
        self.test_loader = DataLoader(
            ESLDatasetByCandidatesWithAudio(test_df, self.criteria, self.audio_processor),
            batch_size=self.batch_size,
            collate_fn=collate_fn,
            num_workers=16,
            prefetch_factor=2,
    ...
    def train(self, use_soft_target=True):
        ...
        for epoch in range(self.epochs):
            ...
            # Improved tqdm progress bar with total count
            ...
            for batch_idx, batch in pbar:
                ...
                pbar.set_postfix({
                    ...
                    'kl': f'{weighted_kl_loss.item():.4f}',
                    'smoothl1': f'{weighted_smoothl1.item():.4f}',
                    'mae': f'{weighted_mae.item():.4f}'
                })
                ...
                if global_step % 10 == 0:
                    wandb.log({
                        ...
                        'train/step_kl_loss': weighted_kl_loss.item(),
                        'train/step_smoothl1_loss': weighted_smoothl1.item(),
                        'train/step_mae': weighted_mae.item(),
                        'train/learning_rate': self.optimizer.param_groups[0]['lr'],
                        'train/global_step': global_step,
                        'train/epoch': epoch + 1
                    }, step=global_step)
            ...
            train_metrics = {
                "epoch": epoch + 1,
                "train/epoch_kl_loss": avg_kl_loss,
                "train/epoch_smooth_mae_loss": avg_mae_loss,
                "train/epoch_total_loss": avg_loss,
                "train/epoch_mae": avg_mae,
                "train/last_lr": lrs[-1] if lrs else 0.0,  # final LR of the epoch
            }

            log_message = f"Epoch {epoch + 1}: Train KLDiv Loss = {avg_kl_loss:.4f}, Weighted MAE Loss = {avg_mae_loss:.4f}, Total Loss = {avg_loss:.4f}, MAE = {avg_mae:.4f}, Learning_rate = {lrs[-1] if lrs else 0.0}"
            print(log_message)
            ...
            val_metrics = {
                "val/weighted_mse": val_w_loss,
                "val/avg_mse": val_avg_loss,
                "val/mae": val_mae,
                "val/best_mae": best_val_mae
            }

            # LOG TO wandb with step
            wandb.log({**train_metrics, **val_metrics}, step=global_step)
            ...
            if val_mae < best_val_mae:
                ...
                checkpoint_path = "./model/model_with_audio_bestmae_Qwen2_15B_newdata_filter_grammar.pth"  # Updated for grammar
                os.makedirs("./model", exist_ok=True)
                self.model.save(checkpoint_path)

                save_message = f"✓ Best model updated at epoch {epoch + 1} with VAL MAE: {val_mae:.4f} -> saved to {checkpoint_path}"
                print(save_message)
                self.logger.info(save_message)

                # THÊM LOG TO wandb
                wandb.log({
                    "val/new_best_mae": val_mae,
...
def main():
    ...
    # Use tokenizer matching the loaded model
    tokenizer = AutoTokenizer.from_pretrained('Alibaba-NLP/gte-Qwen2-1.5B-instruct', trust_remote_code=True)

    # Setup training parameters - using V2 balanced data
    criteria = 'grammar'  # Training on grammar score
    train_df = pd.read_csv("/home/user06/data/Speaking_VSTEP/Test_V2/train_V2_balance.csv")
    batch_size = 2  # Optimized for A100 80GB
    epochs = 5  # Reasonable for continue training
    accumulation_steps = 8  # Effective batch size = 16
    steps_per_epoch = len(train_df) // batch_size
    total_steps = steps_per_epoch * epochs
    warmup_steps = 50  # Shorter warmup for continue training

    # Initialize wandb
    wandb.init(
        dir='./wandb',
        project="esl-audio-grading",
        name=f"V2_grammar_continue_training_{timestamp}",
        config={
            "model_name": "Alibaba-NLP/gte-Qwen2-1.5B-instruct",
            ...
            "regression_dropout": 0.5,
            "criteria": criteria,
            "data_version": "V2_balanced",
            "checkpoint": "grammar_model_continue",
    ...
    # Lower learning rates for continue training (5x lower than initial training)
    param_groups = get_param_groups(model, base_lr=2e-5, encoder_lr=2e-6, scale_lr=2e-4)
    # Freeze the encoder
    ...
    # Initialize trainer with V2 data paths and grammar criterion
    trainer = ESLTrainerByCandidatesWithAudio(
        train_path="/home/user06/data/Speaking_VSTEP/Test_V2/train_V2_balance.csv",
        test_path="/home/user06/data/Speaking_VSTEP/Test_V2/test_V2.csv",
        val_path="/home/user06/data/Speaking_VSTEP/Test_V2/val_V2.csv",
        model=model,
        tokenizer=tokenizer,
        criteria=criteria,  # Pass grammar criterion
    ...
    os.makedirs("./model", exist_ok=True)
    save_path = f"./model/model_V2_grammar_from_vocab_balanced_{timestamp}.pth"
