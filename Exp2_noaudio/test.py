...
from dataloader import ESLDatasetByCandidatesWithAudio, get_collate_fn_bycandidates_without_audio
...
def build_dataloader(csv_path: str, cfg: Config, tokenizer, audio_processor):
    ...
    collate_fn = get_collate_fn_bycandidates_without_audio(
        tokenizer,
        ...
        encoder_type=cfg.model.audio_encoder_type,  # Pass encoder type
        max_mel_bins=128,  # For Whisper
        max_time_steps=3000,  # For Whisper
    )
...
def eval_split(
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            if batch['audio'] is not None:
                ...
                audio = None # Skip audio for testing 
    ...
    metrics = {
        ...
        "edge_pred_ratio": float(edge_pred_ratio),
        "edge_true_ratio": float(edge_true_ratio),
...
def main():
    ...
    test_path = out_dir / f"test_predictions_{cfg.data.criteria}.csv"
    ...
    summary_path = out_dir / f"metrics_summary_{cfg.data.criteria}.json"
    import json
