        ...
        for idx, (raw_text, question_types) in enumerate(zip(raw_texts, self.question_types)):
            ...
            for i, (t, qtype) in enumerate(zip(raw_text, question_types)):
                ...
                if self.questions is not None and idx < len(self.questions):
                    ...
                    question_text = self.question_type_map.get(qtype, '')

                # Format: Question first, then prefix, then response
                formatted_text = (
                    f"{self.question_prefix}{question_text}\n"
                    f"{self.text_prefix}\n"
                    f"{t}"
                )
...
def get_collate_fn_bycandidates_without_audio(tokenizer, max_length=8192,
                                            max_audio_chunks=30,
                                            max_waveform_len=288000,
                                            separate_tokenize=False,
                                            encoder_type='wav2vec2',
                                            max_mel_bins=128,
                                            max_time_steps=3000):
    """
    Collate function with fixed padding for consistent tensor shapes (STEP 3: supports separate tokenization)
    Audio part is loaded by zero tensors.
    ...
    Args:
        ...
        max_waveform_len: Fixed waveform length (288000 = 18s × 16kHz)
        separate_tokenize: Whether to tokenize questions and responses separately (STEP 3)
        encoder_type: 'wav2vec2' for 2D audio or 'whisper' for 3D audio
        max_mel_bins: Max mel bins for Whisper (default 128)
        max_time_steps: Max time steps for Whisper (default 3000)
    ...
    def collate_fn(batch):
        ...
        for item in batch:
            ...
            # ----------- Audio part (all zeros) -------------
            # Create dummy audio tensor based on encoder type
            if encoder_type == 'whisper':
                # Whisper: 3D tensor [chunks, mel_bins, time_steps]
                padded = torch.zeros((max_audio_chunks, max_mel_bins, max_time_steps), dtype=torch.float)
            else:
                # Wav2Vec2: 2D tensor [chunks, waveform_len]
                padded = torch.zeros((max_audio_chunks, max_waveform_len), dtype=torch.float)
            cand_audios.append(padded)
        ...
        audio_tensor = torch.stack(cand_audios, dim=0)  # [B, max_chunks, ...] (2D or 3D)
        score_tensor = torch.stack(scores) if isinstance(scores[0], torch.Tensor) else torch.tensor(scores, dtype=torch.float)
...
if __name__ == "__main__":
    ...
    collate = get_collate_fn_bycandidates_without_audio(tokenizer, max_length=512, max_audio_chunks=18, max_waveform_len=480000, encoder_type='wav2vec2')

    # Create dummy batch with Wav2Vec2 audio [chunks, waveform_len]
    batch_wav2vec2 = [
        {
            'text': ['Part 1 text', 'Part 2 text', 'Part 3 text'],
            'audio': [
                torch.randn(18, 480000),  # 18 chunks, 480000 waveform length
                torch.randn(18, 480000),
                torch.randn(18, 480000)
            ],
            'score': 7.5,
            'question_type': [1, 2, 3],
    ...
    collate_whisper = get_collate_fn_bycandidates_without_audio(tokenizer, max_length=512, max_audio_chunks=18, encoder_type='whisper', max_mel_bins=128, max_time_steps=3000)
    ...
    result = collate_whisper(batch_whisper)
    print(f"   ✓ Input IDs shape: {result['input_ids'].shape}")
    print(f"   ✓ Audio shape: {result['audio'].shape}")
